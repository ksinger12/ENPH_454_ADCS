...
/////////////////////////////
#define nidecHalleffect 12  // External interrupt D12 for Nidec FG 
output 6 Pulses/Revoltion
volatile long countHall = 0;// Used to calculate the pulse value 
calculated by the Hall encoder 
...
void setup() 
{
  ...
  digitalWrite(ledsDirection,HIGH); //Red LED ON // Reaction Wheel home 
resting position
...
void ISR_Routine()
{
  ...
  countpulse();        //pulse plus subfunction // CAN I USE THIS FOR 
ENCODER?
...
differential of the prior estimate error
  ...
  angle_speed = gyro_m - q_bias;   // The differential of the output 
value gives the optimal angular velocity
...
saturation
  PI_pwm = ki_speed * (targetAngle - positions) + kp_speed * 
(targetAngle - speeds_filter);      //speed loop control PI
}
////////////////// End of Speed PI_pwm ////////////////////
