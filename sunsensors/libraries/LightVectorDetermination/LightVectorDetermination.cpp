...
float LightVectorDetermination::get_global_angle(double * voltages) {
    ...
    for (int i = 0; i < n_photodiode; i++)
        pred_angles[i] = photo_model.get_angle(voltages[i]);
...
float LightVectorDetermination::get_fine_angle(){
  ...
  read_photodiode_array(voltages, 2, 4);
  for (int i = 0; i < 2; i++)
    pred_angles[i] = photo_model.get_angle(voltages[i]);
...
void LightVectorDetermination::read_photodiode_array(double ** voltages, int col) {
    /* Populates 2D voltage array with photodiode data and prints to bt in csv form (used for calibration) */
    for (int i = 0; i < n_photodiode; i++) {
        ...
        if (i == n_photodiode - 1) // last photodiode - print new line
            bt.println(temp_row[col]);
        else {
            bt.print(temp_row[col]);
            bt.print(",");
        }
    }
}
...
void LightVectorDetermination::auto_calibrate() {
    ...
    double resolution = TWO_PI / (double)n_readings;                                        			// In radians
    double angular_velocity = get_z_gryo();                                         			// In radians/second
    unsigned long long dt = (unsigned long long)(1000.0*resolution / angular_velocity);    		// In seconds
    // There should be a check for division by zero/low angular velocity.

    bt.println("CALIBRATING...");
    bt.print("Angular Velocity: ");
    bt.print(angular_velocity);
    bt.println(" radians/second");

    bt.print("Angular Resolution: ");
    bt.print(resolution);
    bt.println(" radians");

    bt.print("Time step: ");
    bt.print((long)dt);
    bt.println(" milliseconds");
    ...
    for(int i=0; i<n_readings; i++) {
        ...
        // Update time step
        dt = (unsigned long long)(1000.0*resolution / get_z_gryo());
...
//        bt.print((long)dt);
//        bt.print("\tCode time:\t");
//        bt.print((long)code_time);
//        bt.print("\tDelay time:\t");
//        bt.println((long)dt-(long)code_time);

        // Check if satellite is spinning too fast
        if(code_time>dt) {
            bt.println("Satellite is spinning too fast to gather samples");
            return;
        }
    ...
    for(int i=0; i<n_readings; i++) {
        for(int j=0; j<n_photodiode; j++) {
            cal_voltages[j][i] = cal_voltages[j][i]/(double)n_avg;
            bt.print(cal_voltages[j][i]);
            bt.print("\t");
        }
        bt.println();
    }

    bt.println("DATA COLLECTION COMPLETE. FITTING...");

    this->fit((double**)cal_voltages);

    bt.println("FITTING COMPLETE. SUN SENSORS CALIBRATED.");
}

double LightVectorDetermination::get_z_gryo() {
    Vector angularVelocitySatellite = mpu.readNormalizeGyro();
    double z_angular_velocity = abs((double)angularVelocitySatellite.ZAxis*(TWO_PI/360.0));
