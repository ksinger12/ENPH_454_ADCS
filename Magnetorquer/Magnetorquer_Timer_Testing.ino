...
void loop()
{
  ...
  if (normGyro.ZAxis < previousVelocity && startTimer) {
  ...
  previousVelocity = normGyro.ZAxis;

  if (normGyro.ZAxis == 0 && !startTimer && endTimer) {
    endTimer = false;
    endTime = millis();
    Serial.println("****************************************");
    Serial.println("Time to slow: ");
